...
- `eprintln!` and `println!` are forbidden in library and agent code (`src/backend/`, `src/tools/`, `src/agent/`). Return warnings as part of function return values and emit them via `StreamEvent::Warn` (backend layer) or `AgentEvent::Warn` (agent layer) where an event channel is available.
