...
2. **Agent Core** (`src/agent.rs`) — Owns conversation history, context files, and skills. Wraps backend streams into `AgentEvent` (TokenReceived | ToolUseReceived | ToolResult | ToolConfirmationRequired | ResponseComplete | Error | Usage). Display-agnostic. Drives agentic tool-use loops up to `max_tool_iterations`. Tool calls within a single assistant turn run concurrently via `join_all`; confirmations are gathered sequentially first, then approved calls execute in parallel. Supports session switching (`load_session`) while preserving non-persisted context prefix (context files, skill definitions). Compaction (`src/agent/compact.rs`) summarises conversation history via a headless sub-agent; when `num_retained_turns > 0`, the last N turns are preserved after the summary with non-text blocks stripped (only `Text` blocks kept).
...
**Session persistence** (`src/session.rs`) — Each session is a SQLite database (via `turso`) identified by a UUIDv7. Conversations are persisted per-message. Sessions can be listed, resumed, and deleted. The session picker in the TUI allows browsing and switching sessions. `ConversationRepo` provides `compact()` (full compaction) and `compact_retaining()` (compaction with retained turns) — both use soft-deletion (`active = 0`) so original content is preserved for audit/recovery.
